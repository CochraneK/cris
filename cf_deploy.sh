#!/usr/bin/env bash
# 部署 CRIS Worker（Cloudflare D1 版，模块格式）
...
DB_ID=$(curl -s -X POST "$API/accounts/$ACCT/d1/database" \
  ...
  -d "{\"name\":\"$DB_NAME\"}" | "$PY" -c "import sys,json;d=json.load(sys.stdin);print(d.get('result',{}).get('uuid','') if d.get('success') else '')")
if [ -z "$DB_ID" ]; then
  DB_ID=$(curl -s "$API/accounts/$ACCT/d1/database" -H "$AUTH" \
    | "$PY" -c "import sys,json;d=json.load(sys.stdin);print([n['uuid'] for n in d.get('result',[]) if n.get('name')=='$DB_NAME'][0])")
fi
echo "    D1 database id = $DB_ID"

echo "==> 2/3 写绑定元数据（ES module 格式 + D1 绑定 DB）"
cat > _cf_meta.json <<EOF
{"main_module":"worker.mjs","compatibility_date":"2025-01-01","bindings":[{"type":"d1","name":"DB","id":"$DB_ID"}]}
EOF

echo "==> 3/3 部署 Worker（模块格式）并绑定 D1"
curl -s --retry 3 -X PUT "$API/accounts/$ACCT/workers/scripts/$SCRIPT" \
  -H "$AUTH" \
  -F "worker.mjs=@worker.js;type=application/javascript" \
