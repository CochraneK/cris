...
50 题 7 点量表，帮你在「男性化 / 女性化」两个**独立**维度上看见自己的交织方式，并被归类为四种类型之一（双性化 / 男性化 / 女性化 / 未分化）。报告按你的类型着色，给出暖心、启发、且客观谨慎的解读。为更通俗，应用对外也常把这两个维度表述为「坚韧果断」与「温柔共情」两种特质。
...
- 📲 **私人找回二维码（提交后才生成）**：真实提交成功后，报告底部才会生成一个**私人**二维码，编码你的专属恢复链接；它**绝不进入**「保存报告」分享图（分享图只含报告本身）。截屏单独保存此码，用手机扫一下即可回到原始作答数据，无需记住任何提取码。
...
| 开始页 | 关于本测验 | 四象限落点 |
| :---: | :---: | :---: |
| <img src="posters/cover.png" width="240" alt="开始页"> | <img src="posters/about.png" width="240" alt="关于本测验"> | <img src="posters/quadrant.png" width="240" alt="四象限落点图"> |

| 你的报告（解读） | 保存与分享 |
| :---: | :---: |
| <img src="posters/interpret.png" width="240" alt="个性化解读报告"> | <img src="posters/save.png" width="240" alt="保存与分享报告"> |
...
**只有真实提交成功后**，报告底部的「私人找回」区才会生成一个**小巧的二维码**，编码你的专属恢复链接（`https://cochranek.github.io/cris/?uid=<你的匿名码>`）。它**刻意不进入**「保存报告」分享图（分享图只含报告本身，绝不带私人钥匙）——请把它**单独**截屏保存。用手机扫这个码即可跳转到恢复页、自动凭 uid 从后端取回你的 50 题原始作答与分数——**无需记住任何提取码**，也不受换设备影响。

- 二维码由页面**本地内置**的 `assets/qrcode.min.js` 生成（已随仓库打包，**无需联网、无 CDN 依赖**）；若加载失败，该区会退化为显示明文提取码，手动抄写同样可找回。
...
- **一键演示（点第 2 题题号「2」）**：自动随机作答并**仅本地生成示例报告**——**不提交、不落库**（演示模式），帮你快速预览报告样式与自己的落点，绝不会写入后端或污染真实统计。
