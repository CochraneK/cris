...
- 🧮 计分：男性化(M)、女性化(F) 各自 20 题均值，以 **4.9** 作为四象限分界，即 BSRI 60 题版**常模中位数**（学界通行做法，源自 Bem 1974 原版大样本中位数）
- 🌐 四象限落点图：横轴 女性化、纵轴 男性化，以 4.9 常模中位数为分界；男蓝、女粉
- 👥 可见**其他填写者的落点**：
  - 接入后端（worker.js / D1）时显示**实时真实累积**的全体落点（按 uid 去重，每行一 respondent）；
  - 纯静态（GitHub Pages）且后端不可达时，仅显示你自己的点（当前未启用示例分布回退，见下方说明）
...
分类（以 4.9 为常模中位数分界，维度均值 ≥4.9 视为该特质「高分」；源自 Bem 1974 原版大样本中位数，学界通行做法）：
- **双性化**：M≥4.9 且 F≥4.9
- **男性化**：M≥4.9 且 F<4.9
- **女性化**：M<4.9 且 F≥4.9
- **未分化**：M<4.9 且 F<4.9
...
- 除了散点图用的聚合落点，后端还会**匿名存每个人 60 题的完整作答**：
  每次提交前端生成随机 `uid`（存浏览器 localStorage，即结果页展示的「数据提取码」），连同 60 题打分一起发到
